...
사내 "AI 챔피언 고급과정" 실습 산출물이며, 대상 문서는 장/절/조 계층으로 구성된
**국가계약법령 3종**(국가를 당사자로 하는 계약에 관한 법률·시행령·시행규칙)입니다.
...
```
① 적재 (문서를 넣을 때 1회)
   국가계약법령 3종 (법률·시행령·시행규칙)
        │  (1) 구조 기반 청킹 — chunker.py (정규식으로 장/절/조/별표 인식)
        ▼
   299개 청크 (법률 49 + 시행령 153 + 시행규칙 97 / 각 청크: content 원문 + embed_text 검색용텍스트 + 메타데이터)
...
② 질의 (질문할 때마다)
   ...
   LLM 생성 — OpenRouter(gpt-4o-mini) 또는 Gemini
        ▼
   답변 + 근거 조문 (예: "경쟁에 부칠 여유가 없는 경우 등에 수의계약 가능… — 시행령 제26조")
```
...
| 구조 | 패턴 | 예 |
| --- | --- | --- |
| 장 | `제N장 <제목>` | 제4장 입찰 및 낙찰절차 |
| 절 | `제N절 <제목>` | 제1절 개인정보의 처리 (국가계약법령엔 절 없음) |
| 조 | `제N조(<제목>)` / `제N조의N(...)` | 제33조(입찰공고), 제5조의2(청렴계약) |
| 항/호/목 | `①②…` / `1. 2.` / `가. 나.` | 조 분할 시 사용 |
| 별표 | `[별표 N]` | `[별표 1](제47조 관련)` (예시 — 현재 적재 문서엔 별표 청크 없음) |
...
| **R9** | **표 이중 인덱싱** — 표 청크는 규칙 기반 **자연어 캡션**으로 임베딩, 답변엔 원본 표 제시 | 마크다운 표 기호(`\|`,`---`)는 임베딩 노이즈. 캡션 예: "별표 1(보유기간 책정 기준표), 제47조 관련 서식, 항목: 보유기간, 대상 파일" (예시 — 현재 적재된 국가계약법령 3종엔 별표 청크가 없어 R9는 미적용 상태) |

**브레드크럼(R7 예시):** 짧은 조문의 맥락을 보강하려고 임베딩 텍스트 앞에 경로를 붙입니다 —
`[제4장 입찰 및 낙찰절차 > 제33조(입찰공고)]` (시행령 실제 청크의 embed_text 첫 줄)

**실측 결과:** 국가계약법령 3종 → **총 299청크** (법률 49 / 시행령 153 / 시행규칙 97, 모두 조문+연혁 청크).
법률은 장/절 구분이 없고, 시행령·시행규칙은 장 구분만 있고 절은 없습니다. 부칙의 조 번호가
본문과 같은 번호를 재사용하는 경우(예: 본문 제1조 "목적" vs 부칙 제1조 "시행일")가 있어,
조번호만으로는 구분되지 않는 점에 유의해야 합니다(벤치마크에서는 문서 구분 키로 보완).
...
질문 ──▶ [1] 쿼리 재작성 (QueryRewriter, LLM 1회)
             "수의계약이 뭐야?" → "수의계약을 할 수 있는 법적 사유 및 절차"
     ──▶ [2] 재작성 질의 임베딩(Gemini)
...
# 4) 문서 적재 (국가계약법령 3종 → Gemini 임베딩 → Supabase)
python load_to_supabase.py 국가를당사자로하는계약에관한법률_제21418호_20260611.md
